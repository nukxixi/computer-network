...
int RecvMessage(char* text)
{
    ...
    while (1)
    {
        recvfrom(serverSocket, BUFFER, sizeof(msg), 0, (sockaddr*)&clientAddr, &clientAddrLen);
        ::memcpy(&msg, BUFFER, sizeof(msg));
        if (msg.getFlags() == SEND_FILE)
        {
            //判断接收的是正确的包吗
            if (ack != int(msg.seq) || msg.akCheckSum() != 1)
            {
                //不是正确的包，重发上一个确认包的ACK，并丢弃该数据包
                cout << "===================重传=================" << endl;
                msg.setFlags(ACK);
                msg.filesize = 0;
                msg.ack = last;
                ...
                cout << "当前最新确认数据包序列号：" << last << endl << endl;
                continue;
            }
        ...
        //是结束吗
        else if (msg.getFlags() == OVER_FILE && msg.akCheckSum() == 1)
        {
            cout << "已完成文件接收！" << endl;
            break;
        }
